...
void showHelp() {
    ...
    std::cout<<"\tOr enter a server number from the server list to set that server as your DNS provider."<<std::endl;
...
void showDNSServersList(std::vector<DNSServer> DNSServers) {
    for (size_t i=0; i<DNSServers.size(); i++)
        std::cout<<"\t"<<i+1<<". Name: "<<DNSServers[i].name<<", Address: ("<<DNSServers[i].IPs[0]<<", "<<DNSServers[i].IPs[1]<<")"<<std::endl;
}
